...
def lookup_email_routing_service(client_name: str, db_type: str, conn=None):
    """
    ...
    # Fallback for FROM email: check environment variables (but no hardcoded default string in the script!)
    if not from_email:
        import os
...
def send_email_outlook(to_emails: str, cc_emails: Optional[str], subject: str, body: str, sender_email: Optional[str] = None, attachments: list = None, reply_to: Optional[str] = None):
    ...
    # Clean 'none' placeholder strings from both To and Cc.
    # IMPORTANT: Never remove dccagent@geopits.com from To: — user may intentionally forward there.
    # Only remove it from Cc: when it is already the sender (prevents self-copy loop).
    ...
    def clean_cc_list(email_str):
        ...
        for e in emails:
            ...
            if e_clean and e_clean != "none":
                # Only suppress dccagent in CC when it is already the sender (loop prevention)
    ...
    # Ensure dccagent@geopits.com is always present in to_emails
    to_list = []
    if to_emails:
    ...
    # Build Graph API attachment list from base64 data
    graph_attachments = []
    if attachments:
    ...
    # 1. Primary Method: Microsoft Graph API Client Credentials Flow (User-specified credentials)
    ...
    # 3. Simulated logging (if everything else fails/is missing)
